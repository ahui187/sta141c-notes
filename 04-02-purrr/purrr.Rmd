...
## R evaluates expression lazily

Related meta programming.
...
# note abc was not defined `substitute` captures the expression
f(abc)
```
...
11:20 %>% detect(~ . %% 7 == 0)
# get the index of the first elementdivisible by 7
...
`cross` expects a list of iteratbles
```{r}
li <- list(
  a = c(7, 8, 9),
  b = c(2, 3, 4)
