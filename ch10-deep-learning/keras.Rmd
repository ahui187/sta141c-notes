...
PS: due to an issue of RStudo, we won't be able to see the progress bar while fitting the model.
...
One way to set it up is to edit the `.Renviron` file by `usethis::r_edit_environ("project")` or just `usethis::r_edit_environ()` and put down, for example
...
- After a few minutes, rstudio server should be avaiable via the external ip of the machine.
